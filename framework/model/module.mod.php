...
function module_check_notinstalled_support($module, $manifest_support) {
	...
	$all_support = array('account', 'wxapp', 'webapp', 'phoneapp', 'welcome', 'xzapp', 'aliapp');
	$notinstalled_support = array();
	$has_notinstalled_support = false;
	foreach ($manifest_support as &$s) {
		if ($s == 'app') {
			$s = 'account';
		}
	}
	foreach ($all_support as $support) {
		if (in_array($support, $manifest_support) && $module["{$support}_support"] != MODULE_SUPPORT_ACCOUNT) {
			$has_notinstalled_support = true;
			$notinstalled_support["{$support}_support"] = MODULE_SUPPORT_ACCOUNT;
		} else {
			$notinstalled_support["{$support}_support"] = MODULE_NONSUPPORT_ACCOUNT;
		}
	}
