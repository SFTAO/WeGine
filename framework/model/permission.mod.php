	...
	if (user_is_vice_founder($user['uid'])) {
		...
		if (empty($_W['isfounder'])) {
			if (!empty($user['owner_uid'])) {
				...
				$vice_users = pdo_getall('users', array('owner_uid' => $user['owner_uid']), 'uid');
				...
				foreach ($vice_users as $key => $vice_user) {
					$user_group_num = uni_owner_account_nums($vice_user['uid'], $role);
					$users_accounts['account_num'] += $user_group_num['account_num'];
					$users_accounts['wxapp_num'] += $user_group_num['wxapp_num'];
					$users_accounts['webapp_num'] += $user_group_num['webapp_num'];
					$users_accounts['phoneapp_num'] += $user_group_num['phoneapp_num'];
					$users_accounts['xzapp_num'] += $user_group_num['xzapp_num'];
					$users_accounts['aliapp_num'] += $user_group_num['aliapp_num'];
				}
				...
				$group['maxaccount'] = intval($group_vice['maxaccount']) - $users_accounts['account_num'];
				$group['maxwxapp'] = intval($group_vice['maxwxapp']) - $users_accounts['wxapp_num'];
				$group['maxwebapp'] = intval($group_vice['maxwebapp']) - $users_accounts['webapp_num'];
				$group['maxphoneapp'] = intval($group_vice['maxphoneapp']) - $users_accounts['phoneapp_num'];
				$group['maxxzapp'] = intval($group_vice['maxxzapp']) - $users_accounts['xzapp_num'];
				$group['maxaliapp'] = intval($group_vice['maxaliapp']) - $users_accounts['aliapp_num'];
			}
		}
	}
